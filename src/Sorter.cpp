...
void Sorter::Quicksort(int a, int b, int part) {
    ...
    int pv = 0;
    switch (part) {
        case 0: pv = Naive_partition(a, b);
            break;
        case 1: pv = Middle_partition(a, b);
            break;
        case 2: pv = Fast_partition(a, b);
            break;
        case 3: pv = Median_partition(a, b);
            break;
    }
    BREAKPOINT;
    m_link->solved(pv);
...
int Sorter::Fast_partition(int a, int b) {
    ...
    if (m_link->cmp_ii(p1, p3)) {
        if (m_link->cmp_ee(pVal1, pVal2)) { if (m_link->cmp_ee(pVal2, pVal3)) { p = p2; } else { p = p3; }}
        else {
            p = p1;
        }
    } else {
        if (m_link->cmp_ee(pVal1, pVal2)) { p = p1; }
        else {
            if (m_link->cmp_ee(pVal2, pVal3)) { p = p2; }
            else {
                p = p3;
            }
        }
...
void Sorter::RadixLSD(int baselg) {
    ...
    int access[M];
    Item *cpy[m_size];
    for (int filter = M - 1, d = 0; (filter <= maxval->getVal()) || (filter & maxval->getVal());
         filter <<= baselg, d += baselg) {
        for (int m = 0; m < M; m++) {
            BREAKPOINT;
            access[m] = 0;
        }
        ...
        for (int n = m_size - 1; n >= 0; n--) {
            BREAKPOINT;
            int k = --access[(cpy[n]->getVal() & filter) >> d];
...
void Sorter::RadixMSDLauncher(int baselg) {
    ...
    int shift = 0;
    int level;
    for (level = 0; (filter <= maxval->getVal()) || (filter & maxval->getVal()); level++) {
        BREAKPOINT;
        filter <<= baselg;
        shift += baselg;
    }
...
void Sorter::RadixMSD(int start, int end, int baselg, int M, int filter, int shift, int level) {
    ...
    for (int n = start; n < end; n++) {
        BREAKPOINT;
        access[(cpy[n - start]->getVal() & filter) >> shift]++;
    }
    for (int m = 1; m < M; m++) {
        BREAKPOINT;
        access[m] += access[m - 1];
    ...
    for (int n = end - 1; n >= start; n--) {
        BREAKPOINT;
        int k = --access[((cpy[n - start]->getVal()) & filter) >> shift] + start;
    ...
    if (level) {
        for (int m = 0; m < M; m++) {
            BREAKPOINT;
            RadixMSD(start + access[m],
                     (m == M - 1) ? end : start + access[m + 1],
                     baselg,
                     M,
                     filter >> baselg,
                     shift - baselg,
...
void Sorter::Bucket() {
    ...
    for (int i = 0; i < m_size; ++i) {
        BREAKPOINT;
        access[m_link->get(i).getVal() / bucketcount]++;
    }
    // Turn into cumulative
    ...
    for (int i = m_size - 1; i >= 0; --i) {
        BREAKPOINT;
        cpy[--access[m_link->get(i).getVal() / bucketcount]] = m_link->send(i);
    }
    for (int i = 0; i < m_size; ++i) {
        BREAKPOINT;
        ...
        for (int decr = lv / 2; decr > 0; decr >>= 1) {
            for (int group = 0; group * lv * 2 < m_size; group++) {
                for (int tailing = 0; (tailing + 1) * decr * 2 < lv * 2; tailing++) {
                    for (int offset = 0; offset < decr; offset++) {
                        BREAKPOINT;
                        m_link->flip_ifn_cmp(lv * group * 2 + decr * (tailing * 2 + 1) + offset,
                                             lv * group * 2 + decr * (tailing + 1) * 2 + offset);
